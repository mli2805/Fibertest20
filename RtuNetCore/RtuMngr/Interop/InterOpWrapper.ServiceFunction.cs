...
public partial class InterOpWrapper
{
    public string? GetOtdrInfo(GetOtdrInfo infoType)
    {
        ...
        logger.Info(Logs.RtuManager, $"Get OTDR info error ={result}!");
        return "";
    }
    ...
    public bool SetBaseForComparison(IntPtr baseSorData)
    {
        ...
        if (result != 0)
            logger.Info(Logs.RtuManager, $"Set base for comparison error={result}!");
        return result == 0;
    }
    ...
    public bool MakeAutoAnalysis(ref IntPtr sorData)
    {
        ...
        if (result != 0)
            logger.Error(Logs.RtuManager, $"MakeAutoAnalysis error={result}!");
        return result == 0;
    }
    ...
    public bool Analyze(ref IntPtr sorData, int analysisMode)
    {
        ...
        if (result != 0)
            logger.Error(Logs.RtuManager, $"Analyze error={result}!");
        return result == 0;
    }
    ...
    public bool InsertIitEvents(ref IntPtr sorData)
    {
        ...
        var result = CppImportDecl.ServiceFunction(cmd, ref reserved, ref sorData);
        if (result != 0)
            logger.Error(Logs.RtuManager, $"InsertIitEvents error={result}!");
        return result == 0;
    }
    ...
    public double GetLinkCharacteristics(out ConnectionParams cp)
    {
        ...
        try
        {
            ...
            if (result != 1)
            {
                logger.Error(Logs.RtuManager, $"GetLinkCharacteristics error={result}!");
            ...
            logger.Info(Logs.RtuManager, $"Link characteristics:  Prm1 = {prm1} ns =>  Lmax {res:F} km");
            logger.Info(Logs.RtuManager,
