    ...
    public class CommonC2DWcfManager : IWcfServiceCommonC2D
    {
        ...
        public async Task<ClientMeasurementVeexResultDto> GetClientMeasurementAsync(GetClientMeasurementDto dto)
        {
            ...
            try
            {
                ...
                dto.ClientIp = _clientIp;
                var channel = wcfConnection.CreateChannel();
                var result = await channel.GetClientMeasurementAsync(dto);
                wcfConnection.Close();
                return result;
            }
            catch (Exception e)
        ...
        public async Task<RequestAnswer> InterruptMeasurementAsync(InterruptMeasurementDto dto)
        {
            ...
            try
            {
                _logFile.AppendLine($@"Sending command to interrupt measurement on RTU {dto.RtuId.First6()}");
                dto.ClientIp = _clientIp;
                var channel = wcfConnection.CreateChannel();
                var result = await channel.InterruptMeasurementAsync(dto);
                wcfConnection.Close();
                return result;
            }
            catch (Exception e)
