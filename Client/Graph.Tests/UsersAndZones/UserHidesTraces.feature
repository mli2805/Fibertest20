...
Scenario: Пользователь скрывает и показывает трассы
	...
	Given Выходит root и входит operator
	...
	When Кликает на карте на иконке RTU2 пункт меню Скрыть трассы

	Then В дереве видны оба RTU и обе трассы
	Then В карте видны оба RTU и трасса1 НО трасса2 от RTU2 не видна
	And Не включенные в трассы элементы НЕ видны

	Given Выходит operator и входит supervisor
	Then В дереве видны оба RTU и обе трассы
	Then В карте видны оба RTU и обе трассы
	And Не включенные в трассы элементы НЕ видны

	Given Выходит supervisor и входит operator
	Then В дереве видны оба RTU и обе трассы
	Then В карте видны оба RTU и обе трассы
	And Не включенные в трассы элементы НЕ видны
	...
	Given Выходит operator и входит root
	When Кликает на карте на иконке RTU1 пункт меню Скрыть трассы

	Then В дереве видны оба RTU и обе трассы
	Then В карте видны оба RTU и трасса2 A трасса1 от RTU1 не видна
	And Не включенные в трассы элементы ВИДНЫ
