...
namespace DbMigrationWpf.Measurements
{
    public class MeasurementsFetcher
    {
        ...
        public async Task TransferMeasurements(GraphModel graphModel, IWcfServiceForClient c2DWcfManager)
        {
            foreach (var pair in graphModel.TracesDictionary)
            {
                ...
                _logFile.AppendLine($"{DateTime.Now}  {listOfMeasurementForOneTrace.Count} measurements for trace {pair.Key} found");
                ...
                foreach (var measurementRecord in listOfMeasurementForOneTrace)
                {
                    ...
                    if (i % 100 == 0)
                    {
                        ...
                        _logFile.AppendLine($"{DateTime.Now}  {i} measurements sent");
                        list = new List<AddMeasurementFromOldBase>();
                    }
                }
                await c2DWcfManager.SendMeas(list);
                _logFile.AppendLine($"{DateTime.Now}  {i} measurements sent");
